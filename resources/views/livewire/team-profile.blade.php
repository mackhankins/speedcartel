<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="text-center mb-12">
        <h1 class="text-4xl font-bold text-gray-900 dark:text-white font-orbitron mb-4">Our Team</h1>
        <p class="text-lg text-gray-600 dark:text-gray-400">Meet our talented riders and racers</p>
    </div>

    <!-- Filters -->
    <div class="bg-white dark:bg-darker-gray rounded-xl shadow-sm p-6 mb-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <x-select
                wire:model.live="filters.skill_level"
                label="Filter by Skill Level"
                placeholder="All Skill Levels"
                :options="collect($skillLevelOptions)->map(fn($label, $value) => ['name' => $label, 'value' => $value])"
                option-label="name"
                option-value="value"
            />
            ...
            <x-select
                ...
                :options="collect($classOptions)->map(fn($label, $value) => ['name' => $label, 'value' => $value])"
                option-label="name"
                option-value="value"
            />
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @forelse($riders as $rider)
            <div class="group bg-white dark:bg-darker-gray rounded-xl overflow-hidden transition-all duration-200 border border-gray-100 dark:border-gray-700 hover:shadow-lg">
                ...
                <div class="relative">
                    ...
                    <div class="absolute -bottom-12 left-6 w-24 h-24 rounded-full overflow-hidden border-4 border-white dark:border-darker-gray shadow-sm">
                ...
                <div class="p-6 pt-16">
                    ...
                    <div class="mb-4">
                        <a href="{{ route('riders.profile', $rider) }}" class="group-hover:text-primary-600">
                            <h3 class="font-orbitron text-xl text-gray-900 dark:text-white">
                                {{ $rider->firstname }} {{ $rider->lastname }}
                            </h3>
                        </a>
                        @if($rider->nickname)
                            <div class="text-sm text-gray-600 dark:text-gray-400 font-medium mt-0.5">
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        @if($currentPlate = $rider->plates()->where('is_current', true)->first())
                            <div class="flex items-center justify-between py-1 border-b border-gray-100 dark:border-gray-700">
                                <div class="text-gray-500 dark:text-gray-400 font-medium">{{ ucfirst($currentPlate->type) }} Plate</div>
                                <div class="text-gray-900 dark:text-white">#{{ $currentPlate->number }}</div>
                            </div>
                        @endif
                    </div>
        ...
        @empty
            <div class="col-span-full text-center py-12">
                <div class="text-gray-500 dark:text-gray-400">No riders found matching your filters.</div>
            </div>
        @endforelse
    </div>
</div> 